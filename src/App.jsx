...
import ALL_AIRPORTS from "./airports.json";
...
export default function GlobeFlightFinder() {
  ...
  function drawRoute(fromCode, toCode) {
    ...
    const a = ALL_AIRPORTS[fromCode], b = ALL_AIRPORTS[toCode];
    ...
    const addLabel = (code) => {
      const airport = ALL_AIRPORTS[code];
      const cv = document.createElement("canvas"); cv.width = 128; cv.height = 40;
      const c = cv.getContext("2d");
      c.font = "bold 22px monospace"; c.fillStyle = "#ffe9b0"; c.textAlign = "center";
      c.fillText(code, 64, 26);
      const spr = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(cv), transparent: true, opacity: 0.95, depthWrite: false }));
      spr.scale.set(0.14, 0.044, 1);
      spr.position.copy(llToVec(airport.lat, airport.lon, R + 0.055));
      routeGroup.add(spr);
    };

    const mk = (code, col) => {
      const ap = ALL_AIRPORTS[code];
      const m = new THREE.Mesh(new THREE.SphereGeometry(0.02, 12, 12), new THREE.MeshBasicMaterial({ color: col }));
      m.position.copy(llToVec(ap.lat, ap.lon, R + 0.006));
...
COMMAND: "${command}"
1. Parse origin/destination/date. Map each city to its primary IATA airport code (any IATA code worldwide is allowed — e.g. IDR for Indore, GOI for Goa, SXR for Srinagar, BHO for Bhopal, etc).
2. Web-search current one-way economy fares across Google Flights, Skyscanner, MakeMyTrip, Cleartrip, ixigo, EaseMyTrip. Include all carriers actually serving the route (IndiGo, Air India, SpiceJet, Vistara, Akasa, Cathay, Emirates, Qatar, etc).
3. Web-search current weather for both cities and active weather on the route (storms, monsoon, fog).
Reply ONLY raw JSON, no fences, short strings, under 600 tokens:
{"from":"IDR","to":"HKG","date":"12 Jul 2026","flights":[{"airline":"IndiGo","platform":"MakeMyTrip","price_inr":18999,"duration":"6h 40m","stops":"1 stop"}],"weather":{"from":"34°C haze","to":"Rain 27°C","enroute":"Monsoon band over Bay of Bengal — expect turbulence"},"insight":"one short booking-timing sentence"}
Max 5 flights cheapest first, integer INR prices. IATA codes MUST be real 3-letter codes.`;
    try {
      ...
      if (!parsed.flights?.length || !ALL_AIRPORTS[parsed.from] || !ALL_AIRPORTS[parsed.to]) throw new Error("bad data");
      parsed.flights.sort((x, y) => x.price_inr - y.price_inr);
      setResult(parsed);
      drawRoute(parsed.from, parsed.to);
    } catch (e) {
      console.error(e);
      setError("Couldn't plot that one. Make sure both cities have an airport with an IATA code. Try with the airport city name (e.g. 'Indore to Hong Kong cheapest').");
        ...
        <div style={{ flex: "0 1 340px", minWidth: 280, borderLeft: `1px solid ${DIM}`, padding: 16, overflowY: "auto" }}>
          ...
          {!result && !error && (
            <div style={{ color: MUTE, fontSize: 13, lineHeight: 1.7 }}>
              Give a command. The AI searches real platforms for the cheapest fare across every airport with an IATA code, checks live route weather, and draws the gold path over real continents — green origin, pulsing destination, white dot flying.
              <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
                {["Indore to Hong Kong cheapest", "Goa to Bangkok next month", "Srinagar to Dubai under ₹20,000", "Bhopal to Singapore in July"].map((ex) => (
          ...
          {result && (
            <div className="up">
              ...
              <div style={{ fontWeight: 800, fontSize: 17, margin: "4px 0 2px" }}>
                {ALL_AIRPORTS[result.from].name} <span style={{ color: AMBER }}>→</span> {ALL_AIRPORTS[result.to].name}
              </div>
              ...
              {result.weather && (
                <div style={{ marginTop: 12, background: PANEL, border: `1px solid ${DIM}`, borderRadius: 10, padding: 13, fontSize: 13 }}>
                  ...
                  <div>🛫 {ALL_AIRPORTS[result.from].name}: {result.weather.from}</div>
                  <div style={{ marginTop: 3 }}>🛬 {ALL_AIRPORTS[result.to].name}: {result.weather.to}</div>
